...
office-specific because the pre-election window $T^{\ell}_{t}$ is office-specific, but it does not vary across municipalities in the case of firms that operate in multiple municipalities.
...
cross-office instruments. This choice concerns the timing of the share
component, not the definition of the cross-office shift. The share
$w^{\mathrm{M}}_{jmp,t}$ measures the pre-municipal-election distribution
of owner-party affiliation in sector $j$ and municipality $m$ for each
party $p$. The cross-office component enters only through
$\Align^{c}_{mpt}$, which records whether party $p$ holds the mayoralty
and, for $c\in\{\mathrm{M{\cdot}P},\mathrm{M{\cdot}G},
\mathrm{M{\cdot}G{\cdot}P}\}$, the relevant higher-tier office(s). Thus
using the mayoral window does not assume that owner affiliation is caused
by mayor--higher-tier alignment, nor that affiliation to the mayor's party
is the only relevant political affinity. It fixes the baseline before the
local officeholder common to every channel is chosen.

The identifying rationale is predetermination with respect to the local
component of the channel. Since mayoral and gubernatorial/presidential
elections alternate every two years, no four-year baseline window is
completely insulated from all electoral politics. A higher-tier
pre-window is a natural robustness check for cross-office channels if
owner affiliation responds mainly to federal or state electoral incentives.
A channel-specific window before the earliest election entering the
current cross-office state is cleaner in the strict sense that it precedes
both offices, but it is also mechanically older and may be a noisier
measure of the relevant party-affiliation stock. A window before the
latest election is not an admissible baseline for the cross-office
channels because it can be post-treatment with respect to the office
chosen earlier.
\end{remark}
...
The window $T^{\ell}_t=[e_\ell(t)-4,e_\ell(t)-1]$ in~\eqref{eq:window} is
\emph{frozen} relative to the post-election alignment shocks: it never overlaps
the post-inauguration interval over which $\Align^{\ell}_{mpt}$ is treated as
...
$\EC^{\mathrm{M}}_{jm,t}=\sum_p w^{\mathrm{M}}_{jmp,t}\in[0,1]$ measures the
fraction of cell $(j,m)$'s pre-window owner mass affiliated with any party; the
residual $\mathrm{slack}_{jm,t}\equiv 1-\EC^{\mathrm{M}}_{jm,t}$ is unaffiliated

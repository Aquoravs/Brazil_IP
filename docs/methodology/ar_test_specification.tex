...
\begin{equation}\label{eq:emp-share}
  s^{\mathrm{emp}}_{jmt} \;\equiv\; \frac{n_{jmt}}{n_{mt}},
  \qquad
  n_{mt} \equiv \sum_{j=1}^{J} n_{jmt},
  \qquad
  \sum_{j=1}^{J} s^{\mathrm{emp}}_{jmt} \;=\; 1,
\end{equation}
where the firm universe is RAIS (Brazil's formal-employment registry).
Whenever no ambiguity arises we abbreviate $s_{jmt}\equiv s^{\mathrm{emp}}_{jmt}$;
the superscript ``emp'' is preserved when contrasting against the BNDES credit
share $s^{\mathrm{B}}_{jmt}$ in~\eqref{eq:bndes-share-notation}. The decision
to fix the firm universe at RAIS, taken in D24 (2026-05-06) and operationalized
by the firm-support hybrid memo (D25, 2026-05-12), restricts the analyzed
channel to \emph{formal-sector composition}: politically driven shifts in
non-formal employment (informal, MEI-unfiled, RAIS-Negativa-only) are by
construction outside $s^{\mathrm{emp}}_{jmt}$. Stacked across $j$,
$s_{mt}\equiv(s_{1mt},\dots,s_{Jmt})'\in\Delta^{J-1}$ lies on the unit simplex;
$\iotaVec' s_{mt}=1$ and $\iotaVec'\Delta s_{mt}=0$ for any change. The
outcome is municipal log real GDP, $Y_{mt}\equiv\log(\mathrm{GDP}_{mt})$, and
$\Delta Y_{mt}\equiv Y_{mt}-Y_{m,t-1}$.

\paragraph{Skeleton construction.}
The panel underlying~\eqref{eq:emp-share} is a \emph{contemporaneous unbalanced}
skeleton: a cell $(j,m,t)$ enters the panel if at least one RAIS firm with sector
$j$ is present in muni $m$ in year $t$. Two operational details follow from the
Phase~0 audits (A0.1--A0.3) of the firm-support hybrid plan:

\begin{enumerate}[leftmargin=*,nosep]
\item \emph{Zero-employee firms.} The audit (A0.2) finds the within-RAIS
zero-employee rate at $0.0000\%$: the reconstructed RAIS panel does not retain
Negativa-equivalent rows, so the contemporaneous-skeleton-includes-zero-employee-firms
provision of the strategy memo is moot under the current extract and the
contemporaneous skeleton is equivalent to a $n_{f,t}\geq 1$ requirement. The
$n_{mt}=0$ drop count is 121 muni-years across 36 munis (0.14\%), with no muni
zero across all 16 years.
\item \emph{RAIS coverage bound.} Owner-CNPJs absent from the reconstructed
RAIS panel constitute $7.64\%$ of admin-observed firm-years (A0.1); this is
the upper bound on the Negativa-recoverable firm mass (not a tight equality,
since some Owner-only firms are non-RAIS-universe, e.g.\ informal/MEI). The
corresponding employment-mass impact, computed by reweighting Owner-only
firms to the contemporaneous-RAIS employment scale (A0.5), is bounded at
$1.83\%$ of municipal employment.
\end{enumerate}
The skeleton admits two sensitivity variants used throughout robustness
(\autoref{subsec:robustness}): \emph{frozen}, which restricts the cell to
firms active in $\tau\in T^{\mathrm{M}}_t$, and \emph{balanced}, which
additionally requires presence in every post-election year of the cycle.
...
office-specific because the pre-election window $T^{\ell}_{t}$ is
office-specific, but it does not vary across municipalities in the case
of firms that operate in multiple municipalities.

\paragraph{Primary channel-specific baseline window.}
For the cross-office instruments, the primary exposure window is the
pre-earliest-election window for the offices entering the channel. Let
\[
  \mathcal{O}(\mathrm{M})=\{\mathrm{M}\},\quad
  \mathcal{O}(\mathrm{M{\cdot}P})=\{\mathrm{M},\mathrm{P}\},\quad
  \mathcal{O}(\mathrm{M{\cdot}G})=\{\mathrm{M},\mathrm{G}\},\quad
  \mathcal{O}(\mathrm{M{\cdot}G{\cdot}P})=\{\mathrm{M},\mathrm{G},\mathrm{P}\}.
\]
Define
\begin{equation}\label{eq:channel-window}
  e_c^{\min}(t)\equiv\min_{\ell\in\mathcal{O}(c)} e_\ell(t),
  \qquad
  T^c_t\equiv [\,e_c^{\min}(t)-4,\ e_c^{\min}(t)-1\,]\cap[2002,2017].
\end{equation}
The channel-specific firm exposure is
\begin{equation}\label{eq:omega-channel}
  \omega^{c}_{fp,t}
  \;\equiv\;
  \frac{\sum_{s\in T^{c}_{t}} L_{f,p,s}}
       {\sum_{s\in T^{c}_{t}} L_{f,s}}.
\end{equation}
For the pure mayoral channel, $T^c_t=T^{\mathrm{M}}_t$. For the
cross-office channels, $T^c_t$ is the last four-year window before the last election among the offices in the channel. The associated pre-period firm support is
\[
  \cF^{\mathrm{pre},c}_{jm,t}
  \;\equiv\;
  \big\{\, f\in\cF(j,m) : L_{f,s}>0
       \text{ for some } s\in T^{c}_{t} \,\big\}.
\]
...
\begin{equation}\label{eq:FA-firm}
  \FA^{c}_{fmt}
  \;\equiv\;
  \sum_{p}\,\omega^{c}_{fp,t}\,\Align^{c}_{mpt},
\end{equation}
i.e.\ firm $f$'s pre-earliest-election baseline exposure for channel
$c$, inner-producted with the channel-$c$ shift in municipality $m$ at
time $t$.

\begin{remark}[Primary exposure window]\label{rem:weights}
The primary specification uses the pre-earliest-election window $T^c_t$
in~\eqref{eq:channel-window}. This choice is based on the predetermination rationale: the baseline exposure should be measured before any of the offices in the channel can turn over, so that it is not contaminated by post-treatment changes in owner-party affiliation. Since mayoral and
gubernatorial/presidential elections alternate every two years, the
pre-earliest window is mechanically older for part of each cycle and may
be a noisier measure of the relevant affiliation stock. We therefore
report a mayoral-window exposure, $w^{\mathrm{M}}_{jmp,t}$ used for all
channels, as the mechanism-aligned robustness. A higher-tier pre-window
is a second timing robustness for cases in which owner affiliation
responds mainly to federal or state electoral incentives.
\end{remark}

\subsection{Sector-level exposure weights}\label{subsec:sector-weights}

Building the sector-level instruments from the firm-level
exposures~\eqref{eq:omega-firm} requires two independent choices: across-sector normalization and within-cell aggregation. The former is the choice of the universe over which the sectoral shares are normalized; the latter is the choice of how to aggregate firm-level exposures within a cell. There are two natural normalization universes ---within-cell (cell is its own universe) or within-municipality (municipality is the universe)--- and three natural aggregation methods ---owner-count, equal-firm, or employment weights. We adopt as primary configuration the within-municipality normalization and owner-count aggregation, and report the rest as robustness. The window choice was fixed
in~\autoref{rem:weights}. The rest of this subsection (i)
defines the within-cell intensity weights, which are the cleanest
notation for the BJS conditions and aggregation identity that
follow; (ii) records that identity; (iii) defines Variants~A and~B.

\paragraph{Sector-level baseline exposure (within-cell intensity, robustness).}
The within-cell intensity-weighted baseline exposure aggregates firm-level
...
For the primary cross-office instruments we use the same construction
with the channel-specific pre-earliest window $T^c_t$:
$w^c_{jmp,t}$ is \eqref{eq:wjmp} with $T^\ell_t$ replaced by
$T^c_t$. For the pure mayoral channel this is exactly
$w^{\mathrm{M}}_{jmp,t}$; for the cross-office channels it is the
exposure measured before the earliest election entering the current
cross-office state. The mayoral-window robustness instead uses
$w^{\mathrm{M}}_{jmp,t}$ for all channels.
...
The notation $N^c_{jm,t}$ denotes the same normalizer evaluated on the
channel-specific support $\cF^{\mathrm{pre},c}_{jm,t}$.

\paragraph{Frozen pre-election window.}
The primary window $T^c_t$ in~\eqref{eq:channel-window} is
\emph{frozen} relative to the post-election alignment shocks: it never overlaps
the post-inauguration interval over which $\Align^{c}_{mpt}$ is treated as
the exogenous shift. This satisfies the predetermined-shares logic of
\citet{goldsmith2020bartik} and supplies the share-level predetermination that
\citet{borusyak2022quasi} use to translate shock-level orthogonality
(\autoref{cond:BJS1}) into unit-level orthogonality. The within-cell sum
$\EC^{c}_{jm,t}=\sum_p w^{c}_{jmp,t}\in[0,1]$ measures the
fraction of cell $(j,m)$'s pre-window owner mass affiliated with any party; the
residual $\mathrm{slack}_{jm,t}\equiv 1-\EC^{c}_{jm,t}$ is unaffiliated
mass that contributes zero to $Z^{c}_{jmt}$. In the language of
\citet[\S4.4]{borusyak2022quasi}, this is the \emph{incomplete shares} setup,
and the cell-level slack must be carried as a control in the muni panel.
Operationally we form the per-cell slack column $\mathtt{slack\_frozen\_mt}$
on the frozen window and include it among the controls in the muni panel
of~\eqref{eq:struct}--\eqref{eq:rf-est}; on/off perturbations are documented
in~\autoref{subsec:robustness}.
...
instrument~\eqref{eq:FA-firm} and the sector-level
instruments~\eqref{eq:Z-M}--\eqref{eq:Z-MGP}.

The intensity-based exposure $w^{c}_{jmp,t}$ in~\eqref{eq:wjmp}
...
$T^{c}_{t}$ and both preserving the shift-share decomposition,
...
pre-window employment as the natural averages over the channel-specific
pre-earliest-election window $T^{c}_t$:
\begin{equation}\label{eq:Ebar}
  \bar E^c_{f,t}\;\equiv\;
   \frac{1}{|T^{c}_t|}\sum_{s\in T^{c}_t} E_{f,s},
  \qquad
  \bar E^c_{jm,t}\;\equiv\;\sum_{f\in\cF(j,m)} \bar E^c_{f,t},
  \qquad
  \bar E^c_{m,t}\;\equiv\;\sum_{j'} \bar E^c_{j'm,t}.
\end{equation}
...
argument that justifies $w^{c}_{jmp,t}$ extends without
modification to any object built from $\bar E$.

\paragraph{Variant A: muni-relative aligned-owner share (primary).}
The primary spec keeps the owner-count primitives but replaces the
within-cell normalization in~\eqref{eq:wjmp} with a muni-level
normalization over affiliated owners. Let
\(
  \bar L^c_{jmp,t}\equiv\sum_{s\in T^{c}_t}
    \sum_{f\in\cF(j,m)} L_{f,p,s}
\)
be the pre-window owner-years in $(j,m)$ affiliated with party $p$, and
\(
  \bar L^{c,\mathrm{affil}}_{m,t}\equiv\sum_{j'}\sum_{p'}\bar L^c_{j'mp',t}
\)
the muni-level pre-window total of owner-years affiliated with any party.
Define
\begin{equation}\label{eq:w-own-rel}
  \tilde w^{c,\mathrm{own}}_{jmp,t}
   \;\equiv\;
   \frac{\bar L^c_{jmp,t}}{\bar L^{c,\mathrm{affil}}_{m,t}},
   \qquad
   \tilde Z^{c,\mathrm{own}}_{jmt}
   \;\equiv\;
   \sum_p \tilde w^{c,\mathrm{own}}_{jmp,t}\,\Align^{c}_{mpt}.
\end{equation}
The share $\tilde w^{c,\mathrm{own}}_{jmp,t}$ is the fraction of the
...
$\bar L^c_{jmp,t}$ that the intensity $w^{c}_{jmp,t}$ discards.
Both numerator and denominator are pre-window quantities, so
$\tilde w^{c,\mathrm{own}}_{jmp,t}$ is predetermined relative to the shift
$\Align^{c}_{mpt}$. Across sectors, $\sum_j \tilde Z^{c,\mathrm{own}}_{jmt}
\in[0,1]$ equals the muni's share of pre-window affiliated owners whose
party is aligned under channel $c$.

\paragraph{Variant B: muni-relative aligned-employment share (within-cell-aggregation robustness).}
This robustness keeps the muni-level normalization but swaps
owner-mass for employment-mass at every
...
\begin{equation}\label{eq:Z-emp-rel}
  \tilde Z^{c,\mathrm{emp}}_{jmt}
   \;\equiv\;
   \frac{\displaystyle\sum_{f\in\cF(j,m)} \bar E^c_{f,t}\,\FA^{c}_{fmt}}
        {\bar E^c_{m,t}}.
\end{equation}
This is the pre-window employment of firms in $(j,m)$ whose owners are
aligned under channel $c$ (with $\FA^{c}_{fmt}\in[0,1]$ as the
...
\begin{equation}\label{eq:Z-emp-rel-ss}
  \tilde Z^{c,\mathrm{emp}}_{jmt}
   \;=\;
   \sum_p \tilde w^{c,\mathrm{emp}}_{jmp,t}\,\Align^{c}_{mpt},
    \qquad
   \tilde w^{c,\mathrm{emp}}_{jmp,t}
   \;\equiv\;
   \frac{\displaystyle\sum_{f\in\cF(j,m)} \bar E^c_{f,t}\,\omega^{c}_{fp,t}}
        {\bar E^c_{m,t}}.
\end{equation}
...
\begin{equation}\label{eq:w-emp-decomp}
  \tilde w^{c,\mathrm{emp}}_{jmp,t}
   \;=\;
    \underbrace{\frac{\bar E^c_{jm,t}}{\bar E^c_{m,t}}}_{\text{sector size}}\;\;
    \underbrace{\bar\omega^{c,E}_{jmp,t}}_{\text{employment-weighted intensity}},
    \qquad
    \bar\omega^{c,E}_{jmp,t}\;\equiv\;
    \sum_{f\in\cF(j,m)} \frac{\bar E^c_{f,t}}{\bar E^c_{jm,t}}\,
    \omega^{c}_{fp,t},
\end{equation}
in which $\bar\omega^{c,E}_{jmp,t}$ is the
...
predetermined; the shift sits exclusively in
...
The base-spec exposure control $\EC^{c}_{jm,t}\equiv\sum_p
w^{c}_{jmp,t}$ adapts mechanically:
$\widetilde\EC^{\mathrm{own}}_{jm,t}\equiv
\sum_p \tilde w^{c,\mathrm{own}}_{jmp,t}$ is cell $(j,m)$'s share of
muni affiliated owners under Variant~A, and
$\widetilde\EC^{\mathrm{emp}}_{jm,t}\equiv
\sum_p \tilde w^{c,\mathrm{emp}}_{jmp,t}$ its employment-mass analogue
under Variant~B. The muni-level political-mass scales
$\log\bar L^{c,\mathrm{affil}}_{m,t}$ (Variant~A) and $\log\bar E^c_{m,t}$
...
The primary instrument in first-stage and AR tables is
$\tilde Z^{c,\mathrm{own}}_{jmt}$ (Variant~A). The intensity-based
$Z^{c}_{jmt}$ in~\eqref{eq:Z-M}--\eqref{eq:Z-MGP} is reported as the
across-sector-normalization robustness and
$\tilde Z^{c,\mathrm{emp}}_{jmt}$ (Variant~B) as the
within-cell-aggregation robustness. The three are non-nested but
share the same identification skeleton; differences across them are
informative about which mass concept predicts sector employment
shares in the data.

\subsection{The shift-share form of the cross-office instruments}

We present the shift-share form using the within-cell intensity
weights $w^{c}_{jmp,t}$ from~\eqref{eq:wjmp}; this is the cleanest
notation for the BJS derivation that follows. The primary instrument
$\tilde Z^{c,\mathrm{own}}_{jmt}$ is the analogous shift-share with
the muni-level weights of~\eqref{eq:w-own-rel} substituted for
$w^{c}_{jmp,t}$. With $w^{c}_{jmp,t}$ and the firm-level
instrument~\eqref{eq:FA-firm} in hand, the four sector-level
...
\begin{align}
  ...
  Z^{\mathrm{M{\cdot}P}}_{jmt}
    &\equiv \sum_{p} w^{\mathrm{M{\cdot}P}}_{jmp,t}\,\Align^{\mathrm{M{\cdot}P}}_{mpt},
    \label{eq:Z-MP}\\
  Z^{\mathrm{M{\cdot}G}}_{jmt}
    &\equiv \sum_{p} w^{\mathrm{M{\cdot}G}}_{jmp,t}\,\Align^{\mathrm{M{\cdot}G}}_{mpt},
    \label{eq:Z-MG}\\
  Z^{\mathrm{M{\cdot}G{\cdot}P}}_{jmt}
    &\equiv \sum_{p} w^{\mathrm{M{\cdot}G{\cdot}P}}_{jmp,t}\,\Align^{\mathrm{M{\cdot}G{\cdot}P}}_{mpt}.
    \label{eq:Z-MGP}
\end{align}
...
\begin{equation}\label{eq:dZ}
  \Delta Z^{c}_{jmt}
  \;\equiv\;
  \sum_{p} w^{c}_{jmp,t}\,\Delta\Align^{c}_{mpt},
  \qquad
...
$w^{c}_{jmp,t}$ (sector-municipality exposure to party $p$, on
the pre-earliest-election channel window) and \emph{shifts}
$\Align^{c}_{mpt}$ (whether $p$ controls the offices defining $c$ at
time $t$). It is the
...
\begin{equation}\label{eq:agg-identity}
  Z^{c}_{jmt}
  \;=\;
  \sum_{p} w^{c}_{jmp,t}\,\Align^{c}_{mpt}
  \;=\;
  \sum_{p}\Bigg[\sum_{f\in\cF^{\mathrm{pre},c}_{jm,t}}
       \frac{L_{f,t}}{N^{c}_{jm,t}}\,\omega^{c}_{fp,t}\Bigg]
       \Align^{c}_{mpt}
  \;=\;
  \sum_{f\in\cF^{\mathrm{pre},c}_{jm,t}}
       \frac{L_{f,t}}{N^{c}_{jm,t}}\,\FA^{c}_{fmt}.
\end{equation}
...
\begin{equation}\label{eq:lvl-cond}
  \E\!\left[\Align^{c}_{mp,e+\tau}\,\big|\,
    \{w^{c}_{jmp,e}\}_{j},\,\mathcal{H}_{m,e-1},\, q_{k}\right]
  \;=\;
  \mu_\tau(q_k),
  \qquad \tau=1,\dots,e'-e,
\end{equation}
...
\begin{equation}\label{eq:chg-cond}
  \E\!\left[\Delta\Align^{c}_{mpt}\,\big|\,
    \{w^{c}_{jmp,t}\}_{j},\,\mathcal{H}_{m,t-1},\, q_{k}\right]
  \;=\;
  \mu(q_k),
\end{equation}
...
the 2002-fixed baseline in $\omega^{c}_{fp,t}$ also reduces the
...
\paragraph{Recipient-class composition of the numerator (D5-op).}
The Phase~0 BNDES recipient audit (A0.4) decomposes total disbursements over
2002--2017 into productive-firm (71.6\%), public-entity (28.3\%),
financial-institution intermediation (0.10\%), and residual (\,$<0.01\%$\,)
classes. The financial-intermediation overlap check confirms no double counting
above $0.018\%$ of total credit in any year. Under D5-op (user, 2026-05-12) the
\emph{primary} volume control aggregates all classes:
\begin{equation}\label{eq:vol-joint}
  \Vol^{\mathrm{joint}}_{mt}
  \;\equiv\;
  \frac{\mathrm{BNDES\_total}_{mt}}{\mathrm{GDP}_{m,2002}},
\end{equation}
with the productive-firm component restricted to RAIS-merged disbursements
to remain internally consistent with the D1 RAIS-only analyzed universe. The
\emph{split-volume} robustness specification enters three terms separately:
(i) \emph{productive-nonRAIS}, private + CNAE recipients without a RAIS row
in year $t$ ($\sim 42\%$ of D3.1's productive aggregate); (ii)
\emph{financial-intermediary}; (iii) \emph{public-entity}. Public-credit
flows outside the BNDES envelope (CEF, BB, BNB, constitutional funds) are
\emph{not} in $\Vol_{mt}$; they are registered as out-of-scope in the
project blueprint (open angle on alternative public-credit channels) and
re-enter the discussion under \autoref{subsec:limitations}.
...
\subsection{Robustness exhibits}\label{subsec:robustness}

The estimation pipeline produces four robustness blocks that the AR test is
evaluated against. Each is documented operationally; the specific
F-statistics and rejection probabilities below are drawn from the Phase~1
exploration (\texttt{explorations/anderson\_rubin/active\_denominator/}) at
the production margin $\texttt{policy\_block\_active}\times\texttt{S3}$ and
the side-by-side $\texttt{cnae\_section}\times\texttt{S3}$.

\paragraph{R1. Denominator variants.}
The three skeleton variants of~\autoref{eq:emp-share} (contemporaneous,
frozen, balanced) are estimated as separate AR tests on the same
instrument stack. Phase~1 finds that all three variants reject the
optimality null at conventional levels, with the headline AR $F$-statistic
on the muni panel at $\approx 2.69$ for the
$\texttt{muni\_id}\hat{\;}\texttt{cnae\_section}$ FE-and-clustering
configuration. The contemporaneous variant is the primary; the frozen and
balanced variants are reported alongside, since their first stages drop
the post-election attrition from balanced-panel restriction
(\,$\sim 52\%$ at cycle~2013).

\paragraph{R2. Margin choice: policy block vs.\ CNAE section.}
The production margin uses 12 firm-side policy blocks
($\texttt{policy\_block\_active}\times\texttt{S3}$, with one block held
out as the simplex base $j_0$), so the active instrument set has
$K_{\mathrm{eff}}=9$ blocks $\times$ at most four cross-office channels.
The side-by-side robustness margin is $\texttt{cnae\_section}\times\texttt{S3}$
($J=51$ active bins). Production AR $F$-statistics are $4.37$
($p=2{\cdot}10^{-4}$) at policy block and $2.05$ ($p=2.1{\cdot}10^{-4}$)
at CNAE section. The Phase~1 standalone diagnostic at policy block (C2.1.5)
reports $F=4.19$; drop-top-1-shock $F=3.37$ ($p<0.001$, reject); drop-top-2
$F=2.72$ ($p<0.01$, reject). Toggling the BHJ \S4.4 cell-level slack
control on and off shifts $F$ by $\Delta F=0.023$ at the production FE
configuration, within the Phase~1 envelope.

\paragraph{R3. Joint vs.\ split volume control.}
The default specification enters $\Vol^{\mathrm{joint}}_{mt}$ from~\eqref{eq:vol-joint};
the robustness specification enters productive-nonRAIS, financial-intermediary,
and public-entity disbursements separately, each normalized by initial-period
GDP. Rejection regions are qualitatively stable at both aggregation margins:
joint and split AR tests both deliver $p\approx 2{\cdot}10^{-4}$. The
financial-intermediary channel coefficient in the split-volume reduced form
is $-0.018$ (significant at the 5\% level), qualitatively matching the
Phase~1 OLS exploration's $-0.105$ on the same channel.

\paragraph{R4. Pre-trend characterization (documentation, not a gate).}
Per the user's 2026-05-12 instruction, pre-trend evidence is documented
rather than used as an admissibility gate. Phase~1 reports: the governor
pre-trend rejection initially flagged in B17 resolves as a specification
artifact under strict-timing windows; the presidential pre-trend is genuine
but moderate ($p<0.005$ on $\Delta\log\mathrm{GDP}$); the mayoral pre-trend
is clean ($p=0.80$). As a stress test, dropping the high-pre-trend
presidential CNAE sections delivers a first-stage $F=53.84$ ($p<10^{-7}$)
on the AR-robust specification. Substantively, the AR identification
condition is conditional mean-independence of the shock~\eqref{eq:lvl-cond}
or~\eqref{eq:chg-cond} rather than zero pre-trends, so the presidential
moderate pre-trend is flagged in~\autoref{subsec:limitations} rather than
treated as a failure.

\subsection{Limitations}\label{subsec:limitations}

\paragraph{L1. RAIS-formal-only bound.}
The endogenous variable $s^{\mathrm{emp}}_{jmt}$ is constructed from the
formal-sector RAIS panel. Politically driven shifts in informal employment,
MEI-unfiled activity, and RAIS-Negativa-only firms are outside the analyzed
channel. The upper bound on the Negativa-recoverable firm mass is $7.64\%$
of admin-observed firm-years (A0.1) and the corresponding employment-mass
impact is $1.83\%$ (A0.5). The bound is small enough that informal-sector
mismeasurement is unlikely to overturn the rejection at the contemporaneous
margin, but the structural claim is in any case narrower than ``the sectoral
mix is GDP-optimal''---it is ``the formal-sector composition channel is
GDP-neutral conditional on volume''.

\paragraph{L2. Other public-credit channels.}
The volume control $\Vol_{mt}$ covers BNDES disbursements only. Parallel
public-credit envelopes---Caixa Econômica Federal (CEF), Banco do Brasil
(BB), Banco do Nordeste (BNB), and the constitutional regional funds (FCO,
FNE, FNO)---are not in $\Vol_{mt}$ and are not in the instrument. If the
political shock drives these channels with the same cross-sector tilt as
$Z^{c}_{jmt}$ generates for BNDES, the AR test attributes their composition
effect to the BNDES channel. This is a known limitation of the formal-channel
estimand and is registered as a future-scope open angle in the project
blueprint.

\paragraph{L3. Pre-trend residual on the presidential channel.}
\autoref{prop:lvlchg} states the formal sufficient condition for AR
identification as conditional mean-independence of the shock from prior
trajectories given $q_k$. A non-zero pre-trend on the presidential channel
indicates that some component of $\Align^{\mathrm{P}}_{pt}$ is forecastable
from pre-period dynamics absorbed by $q_k$. With state$\times$year fixed
effects in $q_k$, the residual presidential pre-trend is moderate
($p<0.005$) and the robustness drop-section exercise (R4) preserves the
rejection. The presidential-channel coefficients are nonetheless to be
read with this qualification, and the cross-office channels
$\mathrm{M{\cdot}P}$ and $\mathrm{M{\cdot}G{\cdot}P}$ inherit the same
qualification through the product structure~\eqref{eq:prod-indep}.

\paragraph{L4. Cluster-robust SEs vs.\ AKM shock-level inference.}
The primary specification reports SEs clustered by municipality
(\autoref{sec:inference}). At $K_{\mathrm{eff}}=9$ policy-block instruments,
the standard two-way muni $+$ year clustering widens AR $t$-statistics by
approximately $50\%$ relative to muni-only clustering, but the headline
$5\%$ rejection survives. The \citet{adao2019shift} shock-level correction,
implemented via the \texttt{ssaggregate} routine, is the right inference
under the BJS framework and is flagged for the published version; it is
not in the current pipeline.

\paragraph{L5. First-stage strength on the emp-share endogenous variable.}
The Phase~1 first stage at script~53 returns an effective $F$ in the
interval $[0.03,3.38]$ for $s^{\mathrm{emp}}_{jmt}$ on the
policy-block stack. This is below the conventional strong-instrument
threshold. The AR test is robust to weak instruments by
construction~\citep{andrews2019weak}---it has correct size for any value
of $\Pi$, including $\Pi=0$---so weak-instrument inference is the right
frame for the headline result. We disclose the first-stage $F$ alongside
the AR statistic and do not report a 2SLS point estimate at this margin.

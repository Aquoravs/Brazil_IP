...
municipality $m$ in year $t$.\footnote{The superscript ``B'' marks the BNDES
share. When the context is unambiguous (the firm/sector first-stage scripts in
\texttt{5\_estimation/}, where credit shares are the outcome) we drop the
superscript and let $s_{jmt}$ refer to credit shares; in this document, with
employment shares as the endogenous variable per D24, the unsuperscripted
$s_{jmt}$ refers to employment shares throughout.} BNDES shares are not the
estimand under the framing of D24; they are the upstream mechanism through
which the political shock is transmitted to employment composition. They enter
the analysis only as auxiliary first-stage objects.
...
\end{equation}
with $t_0$ the initial period of the panel ($t_0=2002$). The specification of
$\Vol_{mt}$ is the working choice and is subject to revision after a
theory/math review.

...
**BNDES channels:** (1) **composition channel** — allocation of credit across sectors within a municipality (the channel of interest); (2) **volume channel** — total credit to firms in a municipality (endogenous; see D10, A10).
...
| **P2** | The Anderson–Rubin test is the chosen inferential framework for the muni-level second stage (handles weak/many instruments, weak-IV-robust). | **DECIDED** | `logs/strategy/strategy_memo_ar_test.md`. |
...
|---|---|---|---|---|---|
| **A1** | BNDES institutional/documentary review: priority sectors, program structure, historical changes in sector targeting. | F0, F3 | HIGH | COMPLETED 2026-05-03 | [`logs/strategy/bndes_allocation_logic.md`](../logs/strategy/bndes_allocation_logic.md) |
...
| **A10** | **AR identification alternatives for composition/volume decomposition.** Four approaches: (1) Pure AR — OLS on both; (2) Partial IV — instrument sector shares only; (3) Full IV — instrument both; (4) Mixed — OLS for shares, IV for total. Source: 2026-04-30 meeting. | F3, F4 | HIGH | PROPOSED | — |
...
| Weight horse race (`explorations/anderson_rubin/a7_weight_comparison/`) | [`logs/plans/2026-05-05_a7-revised-weight-comparison.md`](../logs/plans/2026-05-05_a7-revised-weight-comparison.md) | COMPLETED 2026-05-05 — winner = `w_owners_muni_univ`; see [`a7_winner_summary.md`](../explorations/anderson_rubin/a7_weight_comparison/output/a7_winner_summary.md) |
...
- **C6** Alternative employment / production-factor data: RAIS unexploited variables (education, age, wages), INEP Censo Escolar, PPM + PAM immediately actionable. PNAD infeasible. Memo: [`logs/data_exploration/c6_employment_sources.md`](../logs/data_exploration/c6_employment_sources.md).
- **C7** Local deflators: no full-coverage muni deflator exists for 2002–2017. Metro IPCA (~13 metros, ~55% of GDP) is the only off-the-shelf option; wage-residual proxy from RAIS is the tractable full-coverage alternative. Memo: [`logs/data_exploration/c7_local_deflators.md`](../logs/data_exploration/c7_local_deflators.md).

### Paper integration
- `paper/regs.tex` is the current authoritative draft of the Specifications section but is **not** `\input{}`-ed by `main.tex`. `main.tex` §5 contains a placeholder ("Connection to three steps thing (deprecated)"). Decide when to merge.

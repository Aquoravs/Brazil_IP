...
Individual session logs saved to `journal/sessions/YYYY-MM-DD_description.md`.
...
Generated only at merge time — not at every commit or PR. Save to `journal/merges/YYYY-MM-DD_[branch-name].md` (folder created on first use).

---

## Research Journal

Append to `journal/research_journal.md` whenever an agent completes work — writing code, drafting a section, producing a review, making an editorial decision, or transitioning between phases.
...
Agent outputs are saved by category, not under a generic `logs/`: dated events go to `journal/`, knowledge memos go to `docs/`, code outputs go to `paper/tables/` or `explorations/<branch>/output/`.

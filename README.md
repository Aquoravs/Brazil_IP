...
2. **Sector-level** (levels + changes): shift-share `Z_*` / `dZ_*` instruments predict BNDES sector share within municipality
3. **Municipality-level second stage**: predicted sectoral reallocation predicts change in GDP per capita
4. **Null hypothesis** (optimality): marginal reallocation has zero GDP effect (beta ~ 0)
...
Main LaTeX manuscript. `main.tex` is the source of truth.
...
| `BNDES_TABLES` | Regression table output | `paper/tables/` |

...
| `anderson_rubin/instrument_combinations` | ACTIVE | 2026-05-14 advisor question: why mayor-crossed channels, and combine vs. run separately (D25). | Monte Carlo: cross-office per-channel design is correct; additive {M,G,P} is the weakest and most exposed set; `{M·G}` alone beats `{M,G,M·G}`; M is the only universally clean anchor. | Present `findings.md` to advisors; optionally run the §7 real-data checks on the `policy_block` panel. | [anderson_rubin/instrument_combinations/findings.md](anderson_rubin/instrument_combinations/findings.md) | supports next design decision |
